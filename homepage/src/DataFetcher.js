App.DataFetcher = (function(){
    ...
    init = function (){
        var input= [{req: "Bibliotheken", kw1: "", kw2: "", author: "", publisher: "", yearMin: "", yearMax: "", pagesMin: "", pagesMax: "", place: ""}],
			data = {
				data: input
			};
		getData(data);
    ...
    getData = function(data){
        ...
        for(var i=0;i<length;i++){
            ...
            url = "http://localhost:3333/api/get/"+data.data[i].req+"/"+data.data[i].kw1+"/"+data.data[i].kw2+"/"+data.data[i].author+"/"+data.data[i].publisher+"/"+data.data[i].place+"//"+minYear+"/"+maxYear+"//"+minPage+"/"+maxPage+"/";
        	console.log("URL", url);
            $.ajax({
                url: url,
                type: "GET",
